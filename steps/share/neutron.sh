...
sudo apt-get install -qqyf python-openstackclient > /dev/null 2>&1 || true
...
if ! openstack subnet show ext-subnet >/dev/null 2>&1; then
  ...
  if ! openstack subnet create --network ext-net \
                               --subnet-range 10.101.0.0/24 --gateway 10.101.0.1 --no-dhcp  \
                               --allocation-pool start=10.101.0.3,end=10.101.0.254 \
